...
export function calculateUserAnalytics(userData: {
	...
	const totalScore = userData.enrolledCourses.reduce(
		(sum, c) => sum + c.score,
		0
	)
	const averageScore =
		userData.enrolledCourses.length > 0
			? totalScore / userData.enrolledCourses.length
			: 0
	const totalStudyTime = userData.enrolledCourses.reduce(
	...
	// Calculate streak
	let streakDays = 0
	const sortedDates = userData.lastActiveDates.sort(
...
export function calculateCourseAnalytics(courseData: {
	...
	const activeStudents = courseData.students.filter(
		s => s.lastActive > weekAgo
	).length
...
export function calculatePlatformAnalytics(data: {
	...
	const growthRate =
		data.historicalData.previousMonthUsers > 0
			? ((data.historicalData.currentMonthUsers -
